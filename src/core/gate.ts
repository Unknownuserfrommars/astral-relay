/**
 * Code 活动指示器（历史名称 Gate），纯内存状态，仅供面板诊断。
 *
 * 实际放行由 request-auth.ts 的逐请求模式凭据验证负责，不能依赖此时间窗。
 *
 * 活动采集：宿主在「构建请求选项」阶段调用 prompt provider，并按 modes /
