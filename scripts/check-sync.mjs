#!/usr/bin/env node
/**
 * prepack 闸门：防止 TS 源码改了但 dist/index.cjs 没重新构建导致的静默失效。
...
async function main() {
  try {
    ...
    console.error("[check-sync] 失败：缺少产物，请先运行 npm run build");
    process.exit(1);
  }
  ...
  const currentHash = createHash("sha256").update(srcHash).update(manifestHash).digest("hex");
